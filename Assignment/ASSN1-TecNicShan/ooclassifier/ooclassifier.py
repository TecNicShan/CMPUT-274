...
class TrainingSet(C274):
    ...
    def preprocess(self, mode=""):
        # training set contains training instances
        # make for loop to run preprocess words on training instance
        # Code from TA Hours Nov.7
    ...
    def return_nfolds(self, num=3):
        ...
        for x in range(0, num):
            # print(x)
            partition.append(TrainingSet())
            partition[x].inObjList.clear()
            partition[x].inObjHash.clear()
        for y in range(len(instance)):
            partition[y % num].inObjHash.append(instance[y])
            partition[y % num].inObjList.append(line[y])
            # print(partition[x % num].inObjHash)
        # print(partition)
        # print(partition[1].inObjList)
        # sys.exit()
        return partition
    ...
    def add_training_set(self, tset):
        instance = copy.deepcopy(tset.get_instances())
        lines = copy.deepcopy(tset.get_lines())
        self.inObjHash.extend(instance)
        self.inObjList.extend(lines)
...
class ClassifyByTopN(ClassifyByTarget):
    ...
    def target_top_n(self, tset, num=5, label=""):
        ...
        last_freq = freq[num-1][1]
        ties = -1
        for x in freq[num-1:]:
            if x[1] == last_freq:
                ties += 1
        # print(words)
        self.set_target_words(words[:num+ties])
